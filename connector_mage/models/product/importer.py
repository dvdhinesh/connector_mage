...
class ProductImportMapper(Component):
    ...
    @mapping
    def magento_supplier_id(self, record):
        if 'SupplierID' in record:
            product = self.env['product.product'].search(
                [('default_code', '=', record['SKU'])])
            supplier = self.env['res.partner'].search(
                [('magento_supplier_id', '=', record['SupplierID'])])
            prod_sup_rel = self.env['product.supplierinfo'].search([
                ('product_tmpl_id', '=', product.product_tmpl_id.id),
                ('name', '=', supplier.id)])
            if not prod_sup_rel:
                return {'seller_ids': [(0, 0, {'name': supplier.id})]}

    @mapping
    def attribute_set(self, record):
        if 'AttributeSet' in record:
            return {'magento_attribute_set':
                    MAGENTO_ATTRIBUTE_SET.get(record['AttributeSet'], False)}

    @mapping
    def battery_capacity(self, record):
        if 'AttributeSet' in record and record['AttributeSet'] == 'Battery':
    ...
    def battery_islipo(self, record):
        if 'AttributeSet' in record and record['AttributeSet'] == 'Battery' and record['Attributes']['IsLipo'] == 'Yes':
            return {'battery_islipo': 1}

    @mapping
    def motor_kv(self, record):
        if 'AttributeSet' in record and record['AttributeSet'] == 'Motor':
    ...
    def plane_channels(self, record):
        if 'AttributeSet' in record and record['AttributeSet'] == 'Plane':
    ...
    def propeller_pitchy(self, record):
        if 'AttributeSet' in record and record['AttributeSet'] == 'Propeller':
    ...
    def servo_speed(self, record):
        if 'AttributeSet' in record and record['AttributeSet'] == 'Servo':
